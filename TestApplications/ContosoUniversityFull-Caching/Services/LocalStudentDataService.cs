...
namespace ContosoUniversityFull.Services
{
    public class LocalStudentDataService : IStudentDataService
    {
        ...
        public LocalStudentDataService(SchoolContext db)
        {
            this.db = db;
        ...
        public async Task UpdateStudentAsync(Student entity, Picture userPicture)
        {
            ...
            var studentToUpdate = await db.Students.Include(s => s.UserPicture).SingleOrDefaultAsync(s => s.ID == entity.ID).ConfigureAwait(false);
